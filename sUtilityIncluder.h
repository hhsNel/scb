...
Including this file, you need to specify the following:)
 ...
 $t (SUI_BASE_NAME)		$t (The base name, to be tweaked by different SUI_ settings)
	 ...
	 $t (SUI_TARGET_0_SUFFIX)	$t (Type 0 suffix)
	 $t (SUI_TARGET_TYPE_1)		$t (Any type. Used by the target)
	 $t (SUI_TARGET_1_SUFFIX)	$t (Type 1 suffix)
	 $t (SUI_TARGET_TYPE_2)		$t (Any type. Used by the target)
	 $t (SUI_TARGET_2_SUFFIX)	$t (Type 2 suffix)
	 $t (SUI_TARGET_TYPE_3)		$t (Any type. Used by the target)
	 $t (SUI_TARGET_3_SUFFIX)	$t (Type 3 suffix)
	 $t (SUI_VARIATE_PTR_0_SAFETY)	$t (If SUI_YES, the target will be included twice with SUI_TARGET_PTR_0 set to ssafeptr and sunsafeptr, and SUI_TARGET_NAME appended with -s and -u. If SUI_NO, SUI_TARGET_PTR_0 will be svoidptr)
	 $t (SUI_VARIATE_PTR_1_SAFETY)	$t (If SUI_YES, the target will be included twice with SUI_TARGET_PTR_1 set to ssafeptr and sunsafeptr, and SUI_TARGET_NAME appended with -s and -u. If SUI_NO, SUI_TARGET_PTR_1 will be svoidptr)
	 $t (SUI_VARIATE_PTR_2_SAFETY)	$t (If SUI_YES, the target will be included twice with SUI_TARGET_PTR_2 set to ssafeptr and sunsafeptr, and SUI_TARGET_NAME appended with -s and -u. If SUI_NO, SUI_TARGET_PTR_2 will be svoidptr)
	 $t (SUI_VARIATE_PTR_3_SAFETY)	$t (If SUI_YES, the target will be included twice with SUI_TARGET_PTR_3 set to ssafeptr and sunsafeptr, and SUI_TARGET_NAME appended with -s and -u. If SUI_NO, SUI_TARGET_PTR_3 will be svoidptr)
*/
